...
class features_extraction(object):
    ...
    def __init__(self, config):
        ...
        self.batch_size = int(config.get('simple_NN', 'train_batch_size'))
    ...
    def save_batch_data(self, kaldi_batch_data, kaldi_batch_labels, step, mini_batch_size):
        ...
        batch_inputs = gzip.GzipFile(self.save_dir+'/batch_inputs_'+str(step)+'_mini_batch_' + mini_batch_size + '.npy.gz', 'w')
        np.save(batch_inputs, processed_batch_inputs)
        batch_inputs.close()
		
        batch_targets = gzip.GzipFile(self.save_dir+'/batch_targets_'+str(step)+'_mini_batch_' + mini_batch_size + '.npy.gz', 'w')
        np.save(batch_targets, processed_batch_targets)
        batch_targets.close()
		
        batch_input_seq_length = gzip.GzipFile(self.save_dir+'/batch_input_seq_length_'+str(step)+'_mini_batch_' + mini_batch_size + '.npy.gz', 'w')
        np.save(batch_input_seq_length, processed_batch_input_seq_length)
        batch_input_seq_length.close()
		
        batch_output_seq_length = gzip.GzipFile(self.save_dir+'/batch_output_seq_length_'+str(step)+'_mini_batch_' + mini_batch_size + '.npy.gz', 'w')

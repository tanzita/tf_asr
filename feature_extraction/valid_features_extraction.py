...
class features_extraction(object):
    ...
    def __init__(self, config, train_important_information):

        self.context_width = int(config.get('simple_NN', 'context_width'))
        self.save_dir = config.get('directories', 'exp_dir') + '/valid_features_dir'
        self.batch_size = int(config.get('simple_NN', 'valid_batch_size'))
        self.pdf_file_dir = config.get('directories', 'valid_pdf_dir')
        self.pdf_file_total = int(config.get('general', 'valid_num_pdf_files'))
        self.ark_file = config.get('general', 'valid_ark')
        self.valid_cmvn_ark = config.get('general', 'valid_cmvn_ark')
        self.cmvn_dict = self.get_cmvn_dict(self.valid_cmvn_ark)

        self.max_length = train_important_information['train_label_max_length']
        self.total_number_of_utterances = 0
        self.input_dim = 0
        self.input_dim_check = False
		
	#change group minibatches
	self.different_batch_sizes = config.get('simple_NN', 'different_minibatches')

        utt_2_spk_file = config.get('general', 'valid_utt_2_spk')
        self.utt_2_spk_dict = self.get_utt_2_spk_dict(utt_2_spk_file)

        self.num_labels = int(config.get('general', 'valid_num_label'))
    ...
    def save_batch_data(self, kaldi_batch_data, kaldi_batch_labels, step):
        ...
        batch_inputs = gzip.GzipFile(self.save_dir+'/batch_inputs_'+str(step)+'.npy.gz', 'w')
        np.save(batch_inputs, processed_batch_inputs)
        batch_inputs.close()
		
        batch_targets = gzip.GzipFile(self.save_dir+'/batch_targets_'+str(step)+'.npy.gz', 'w')
        np.save(batch_targets, processed_batch_targets)
        batch_targets.close()
		
        batch_input_seq_length = gzip.GzipFile(self.save_dir+'/batch_input_seq_length_'+str(step)+'.npy.gz', 'w')
        np.save(batch_input_seq_length, processed_batch_input_seq_length)
        batch_input_seq_length.close()
		
        batch_output_seq_length = gzip.GzipFile(self.save_dir+'/batch_output_seq_length_'+str(step)+'.npy.gz', 'w')
    ...
    def batch_data_processing(self):
        ...
        #change group minibatches
		different_minibatches = self.different_batch_sizes.split()
		
		print different_minibatches
        ...
        #change group minibatches
	for batch_sizes in different_minibatches:
		
	    if batch_sizes:
			
		batch_size_value = int (batch_sizes)
		    ...
		    #change group minibatches
                    if (id_count + 1) % batch_size_value == 0:
					
                        #change group minibatches
                        self.save_batch_data(utt_mat, target_mat, batch_count, batch_sizes)
                        utt_mat = []
                        target_mat = []
                        batch_count += 1
        ...
        important_info = {'valid_utt_max_length': self.max_length, 
                   'valid_batch_total': batch_count - 1, 
                   'total_valid_utterances': self.total_number_of_utterances, 
                   'input_dim': self.input_dim,
                   'num_labels':self.num_labels,
                   'valid_label_max_length':self.max_target_length}

        with open(self.save_dir+"/valid_important_info", "wb") as fp:
...
def get_important_info(save_dir):
    ...
    with open(save_dir+"/valid_important_info", "rb") as fp:
